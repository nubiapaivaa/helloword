...
class _CameraPageState extends State<CameraPage> {
  ...
  Future<void> _inicializarCamera({
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _carregandoCamera = false;
          _erro =
              'Não foi possível acessar a câmera: ${exception.code}.';
        });
      }
    }
  }
  ...
  Future<void> _alternarCamera() async {
    if (_cameras.length < 2 || _carregandoCamera) {
      return;
    }
  ...
  Future<void> _tirarFoto() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _erro =
              'Não foi possível tirar a foto: ${exception.code}.';
        });
      }
    }
  }
        ...
        children: [
          Row(
            children: [
              ...
              IconButton(
                onPressed:
                    _cameras.length < 2 ? null : _alternarCamera,
          ...
          const Text(
            'Tire uma foto para comprovar a realização do treino.',
          ),
          ...
          FilledButton.icon(
            onPressed:
                controller == null ? null : _tirarFoto,
